...
for n_gp in 0:2
    ...
    # @jzhao
    # plotting GLOM on individual indicators
    if individual_gp == true
        plt1 = scatter(obs_xs, rvs_and_inds[1], yerror=rvs_and_inds_err[1], xlabel="obs_xs", ylabel="indicator_"*string(n_gp), label="data")
        plot!(plt1, plot_xs, GLOM_at_plot_xs[1], ribbons=GLOM_err_at_plot_xs[1], fillalpha=0.3, label="model")
        plt2 = scatter(obs_xs, GLOM_at_obs_xs[1] - rvs_and_inds[1], yerror=rvs_and_inds_err[1], label="model - data", xlabel="obs_xs", ylabel="residual")
        plot(plt1, plt2, layout = @layout [a; b])
        filename_fig = joinpath(string(STAR_ID) * "_pennstate" * "_glom_" * kernel_name * "_indictor_"*string(n_gp)*".png")
    ...
    # println("log likelihood for GLOM + planet model: " * string(-fit_nlogL2)) @jzhao

    println("\nunnormalized posterior for GLOM model: " * string(uE1))
    # println("unnormalized posterior for GLOM + planet model: " * string(uE2)) @jzhao

    println("\nevidence for GLOM model: " * string(E1))
    # println("evidence for GLOM + planet model: " * string(E2)) @jzhao

    # # This should be pretty close to true
    # GLOM_RV.test_∇∇nlogL_kep(problem_definition_rv, workspace.Σ_obs, current_ks; include_priors=true)


    # The evidence shows the marginal likelihood of a model with a certain number of activity indicators


    # The individual evidence shows the marginal likelihood of a GLOM model on the given activity indicators
    # obtained by substituting the obs_rvs by the activy indicators
    if individual_gp == true
        open(STAR_ID*"_individual_evidence_"*string(n_gp)*".txt", "w") do io
                   writedlm(io, E1)
        end
    else
        open(STAR_ID*"_evidence_"*string(n_gp+1)*".txt", "w") do io
                   writedlm(io, E1)
        end
    end
end
